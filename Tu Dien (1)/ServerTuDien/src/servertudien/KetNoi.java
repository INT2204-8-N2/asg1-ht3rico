...
{   //Neu chua tao ket noi thi khoi tao ket noi den MySQL
    if(this.conect==null)
    {
      ...
      try
      {
         this.conect= DriverManager.getConnection(url,this.Username, this.Password);//ket noi db
  ...
  protected Statement getStatement() throws Exception {  //ham thuc thi cau lenh Query
      // Neu staement =null hoac da dong thi can khoi tao lai
        if (this.statement == null ? true : this.statement.isClosed()) {
            this.statement = this.getConect().createStatement();
        }
        return statement;
    }
  ...
  public ResultSet excuteQuery(String query) throws Exception {
    ...
    //nếu result chưa đóng thì đóng result
        ...
        if(this.conect!=null&& !this.conect.isClosed())
        {
            this.conect.close();
            this.conect=null;
        }
